...
private val BLE_BYTE_ORDER: ByteOrder = ByteOrder.BIG_ENDIAN
...
fun Short.toByteArr(): ByteArray {
    return ByteBuffer.allocate(2).order(BLE_BYTE_ORDER)
        .putShort(this).array()
}

fun Int.toByteArr(): ByteArray {
    return ByteBuffer.allocate(4).order(BLE_BYTE_ORDER)
        .putInt(this).array()
}
...
fun ByteArray.toUnsignedInt(): Long {
    ...
    return ByteBuffer.wrap(byteArray)
        .order(BLE_BYTE_ORDER).int.toLong() and 0xffffffffL
}
